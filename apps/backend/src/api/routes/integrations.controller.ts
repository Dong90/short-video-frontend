...
export class IntegrationsController {
  ...
  async connectSocialMedia(
    ...
    const createdIntegration = await this._integrationService.createOrUpdateIntegration(
      additionalSettings,
    ...
    // 添加频道成功后，在 short_video 中创建 platform_account（仅支持的平台）
    if (this._shortVideoSync.shouldSyncForProvider(integration)) {
      try {
        await this._shortVideoSync.createPlatformAccountForIntegration({
        ...
        console.warn('Failed to create short_video platform_account:', e?.message);
      }
    }
  ...
  async deleteChannel(
    ...
    // 删除频道时，同步删除 short_video 中关联的 platform_accounts（所有频道类型）
    await this._shortVideoSync.deletePlatformAccountsByIntegrationId(id);
