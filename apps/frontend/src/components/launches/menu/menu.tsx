...
import { ShortVideoInfoModal } from '@gitroom/frontend/components/short-video';
import copy from 'copy-to-clipboard';

// 支持短视频的平台
const SHORT_VIDEO_PLATFORMS = ['youtube', 'youtube-channel', 'tiktok', 'instagram', 'douyin'];
...
export const Menu: FC<{
  ...
  const openShortVideoInfo = useCallback(() => {
    const integration = integrations.find((i) => i.id === id);
    if (!integration) return;
    modal.openModal({
      classNames: {
        modal: 'w-[100%] max-w-[920px] bg-transparent',
      },
      withCloseButton: false,
      closeOnEscape: true,
      closeOnClickOutside: true,
    ...
    >
      ...
      {show && (
        ...
        >
          ...
          {SHORT_VIDEO_PLATFORMS.includes(findIntegration?.identifier) && (
            <div
              className="flex gap-[12px] items-center py-[8px] px-[10px] cursor-pointer hover:bg-newSettings/50 rounded-[6px] -mx-[2px]"
              onClick={openShortVideoInfo}
            >
              <div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <div className="text-[14px]">{t('short_video', '短视频')}</div>
            </div>
          )}
          {canDisable &&
