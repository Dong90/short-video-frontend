...
import { ShortVideoTasksPanel } from '@gitroom/frontend/components/new-launch/short-video.tasks.panel';
import type { AddEditModalMode } from '@gitroom/frontend/components/new-launch/add.edit.modal';
...
export const ManageModal: FC<AddEditModalProps> = (props) => {
  ...
  // Tab 模式：发帖 | 短视频任务，根据 initialMode 初始化
  const [mode, setMode] = useState<AddEditModalMode>(
    props.initialMode || 'post'
  );
      ...
      <div className="flex flex-1 bg-newBgColorInner rounded-[20px] flex-col">
        <div className="flex-1 flex">
          <div className="flex flex-col flex-1 border-e border-newBorder">
            {/* 方案 B：顶部 Tab 发帖 | 短视频任务 */}
            <div className="bg-newBgColor h-[65px] rounded-s-[20px] !rounded-b-[0] flex items-center justify-between px-[20px] gap-[8px]">
              <div className="flex rounded-[8px] bg-newBgColorInner p-[4px] gap-[4px]">
                {!existingData.integration && (
                  <>
                    <button
                      type="button"
                      ...
                      className={clsx(
                        'px-[16px] py-[8px] rounded-[6px] text-[14px] font-[600] transition-colors',
                        mode === 'post'
                          ? 'bg-[#612BD3] text-white'
                          : 'text-textItemBlur hover:text-textColor'
                      )}
                    >
                      {t('create_post', '发帖')}
                    </button>
                    ...
                    >
                      {t('short_video_task', '短视频任务')}
                    </button>
                  </>
                )}
                {existingData.integration && (
                  <div className="px-[16px] py-[8px] text-[14px] font-[600] text-textColor">
                    {t('create_post_title', 'Create Post')}
                  </div>
                )}
              </div>
              {/* 关闭按钮：短视频模式下右侧预览区隐藏，需在顶部显示 */}
            ...
            <div className="flex-1 flex flex-col gap-[16px]">
              ...
              >
                ...
                {/* 短视频任务模式 */}
                {mode === 'short_video' && (
                  <div className="absolute top-0 left-0 w-full h-full overflow-x-hidden overflow-y-scroll scrollbar scrollbar-thumb-newColColor scrollbar-track-newBgColorInner pt-[20px] ps-[20px] pe-[20px] flex flex-col gap-[16px]">
