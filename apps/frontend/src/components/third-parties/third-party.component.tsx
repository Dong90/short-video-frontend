...
import { ShortVideoIntegrationSettings } from '@gitroom/frontend/components/short-video';
...
export const ThirdPartyComponent = () => {
  ...
  const openShortVideoSettings = useCallback(() => {
    modals.openModal({
      title: '',
        ...
        <div className="flex flex-col gap-[4px] mb-[8px]">
          <div className="flex items-center justify-between gap-[8px]">
            <div className="text-[16px] font-[500]">
              {t('third_party_integrations', '第三方平台 / 集成')}
            </div>
            <button
              type="button"
              onClick={openShortVideoSettings}
              className="h-[28px] px-[10px] rounded-[6px] border border-newBorder text-[12px] text-textItemBlur hover:bg-newBgColor"
            >
              {t('short_video_settings_entry', '短视频平台设置')}
            </button>
          </div>
          <div className="text-[12px] text-textItemBlur">
            {t(
              'short_video_integration_hint',
              '在这里添加或管理第三方平台账号'
            )}
          </div>
        </div>
