...
export class IntegrationRepository {
  ...
  async updateIntegration(id: string, params: Partial<Integration>) {
    if (params.picture && typeof params.picture === 'string') {
      const cloudflare = process.env.CLOUDFLARE_BUCKET_URL || '';
      const frontend = process.env.FRONTEND_URL || '';
      const isAlreadyOurs =
        (cloudflare && params.picture.indexOf(cloudflare) > -1) ||
        (frontend && params.picture.indexOf(frontend) > -1) ||
        params.picture.indexOf('imagedelivery.net') > -1;
      if (!isAlreadyOurs && params.picture.startsWith('http')) {
        try {
          params.picture = await this.storage.uploadSimple(params.picture);
        } catch (e: any) {
          console.warn('updateIntegration: upload picture failed, keeping original URL:', e?.message);
        }
      }
    }
