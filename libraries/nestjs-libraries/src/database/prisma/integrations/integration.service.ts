...
export class IntegrationService {
  ...
  constructor(
    ...
    @Optional()
    @Inject('SHORT_VIDEO_ON_DISCONNECT')
    private _onChannelDisconnect?: (integrationId: string) => void | Promise<void>
  ) {}
  ...
  async createOrUpdateIntegration(
    ...
    if (picture) {
      const isAlreadyOurs =
        ...
        (process.env.FRONTEND_URL && picture.indexOf(process.env.FRONTEND_URL) > -1);
      if (isAlreadyOurs) {
        ...
        try {
          ...
          console.warn('createOrUpdateIntegration: upload picture failed, using original URL:', e?.message);
          uploadedPicture = picture.startsWith('http') ? picture : undefined;
        }
      }
    }
  ...
  async disconnectChannel(orgId: string, integration: Integration) {
    ...
    // 频道失效时，同步删除 short_video 中关联的 platform_accounts
    if (this._onChannelDisconnect) {
      void Promise.resolve(this._onChannelDisconnect(integration.id)).catch((e) => {
        console.warn('SHORT_VIDEO_ON_DISCONNECT callback failed:', e?.message);
      });
    }
  }
